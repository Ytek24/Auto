...
public class FilterAndResearch {
    ...
    private ArrayList<Date> releaseDateList;
    private ArrayList<Color> colorList;
    private ArrayList<Integer> nbSeatsList;
    private ArrayList<Integer> fiscalPowerList;
    private ArrayList<Double> consumptionList;
    private ArrayList<Double> co2EmissionList;
    private ArrayList<Double> distanceTraveledList;
    private ArrayList<Double> priceList;
    private ArrayList<Integer> nbWheelsList;
    ...
    public static Catalog createFilteredCatalog(Catalog catalog, String brand,
    String model/*, Date minReleasedDate, Date maxReleasedDate*/, boolean yesSecondHand,
    boolean noSecondHand, int nbSeats, int minFiscalPower, int maxFiscalPower,
    double minConsumption, double maxConsumption, double minCo2Emission,
    double maxCo2Emission, double minDistanceTraveled, double maxDistanceTraveled,
    double minPrice, double maxPrice)
    {
        Catalog catalogFiltered = new Catalog();

        for(VehicleCatalog vehicleCatalog: catalog.getVehiclesCatalog())
        {
            if((vehicleCatalog.getBrand().compareTo(brand) == 0) || (brand.compareTo("") == 0))
            {
                if((vehicleCatalog.getModel().compareTo(model) == 0) || (model.compareTo("") == 0))
                {
                    if((!yesSecondHand && !noSecondHand) ||
                        (vehicleCatalog.isSecondHand() && yesSecondHand) ||
                            (!vehicleCatalog.isSecondHand() && noSecondHand))
                    {
                        if((vehicleCatalog.getNbSeats() == nbSeats) || (nbSeats == -1))
                        {
                            if(vehicleCatalog.getFiscalPower() >= minFiscalPower &&
                                    vehicleCatalog.getFiscalPower() <= maxFiscalPower)
                            {
                                if(vehicleCatalog.getConsumption() >= minConsumption &&
                                vehicleCatalog.getConsumption() <= maxConsumption)
                                {
                                    if(vehicleCatalog.getCo2Emission() >= minCo2Emission &&
                                    vehicleCatalog.getCo2Emission() <= maxCo2Emission)
                                    {
                                        if(vehicleCatalog.getDistanceTraveled() >= minDistanceTraveled &&
                                        vehicleCatalog.getDistanceTraveled() <= maxDistanceTraveled)
                                        {
                                            if(vehicleCatalog.getPrice() >= minPrice &&
                                            vehicleCatalog.getPrice() <= maxPrice)
                                            {
                                                catalogFiltered.addVehicleCatalog(vehicleCatalog);
                                            }
                                        }
                                    }
                                }
    ...
    public ArrayList<Date> getReleaseDateList() {
    ...
    public ArrayList<Integer> getNbSeatsList() {
        return nbSeatsList;
    }

    public ArrayList<Integer> getFiscalPowerList() {
    ...
    public ArrayList<Double> getCo2EmissionList() {
        return co2EmissionList;
    }

    public ArrayList<Double> getDistanceTraveledList() {
    ...
    public ArrayList<Integer> getNbWheelsList() {
        return nbWheelsList;
    }
}
